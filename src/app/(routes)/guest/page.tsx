  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* Welcome Section */}
        <div className="relative overflow-hidden bg-gradient-to-br from-emerald-500 via-green-500 to-teal-600 rounded-2xl p-6 text-white shadow-xl">
          <div className="absolute top-0 right-0 w-48 h-48 bg-white/10 rounded-full -translate-y-24 translate-x-24 blur-3xl"></div>
          <div className="absolute bottom-0 left-0 w-36 h-36 bg-white/10 rounded-full translate-y-18 -translate-x-18 blur-3xl"></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat, index) => {
            ...
            return (
              <div 
                key={index} 
                className={`bg-gradient-to-br ${stat.bgGradient} rounded-xl p-4 shadow-sm border ${stat.borderColor} hover:shadow-lg transition-all duration-300 hover:scale-105 group cursor-pointer`}
              >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          {/* Recent Bookings */}
          <div className="xl:col-span-2 bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-200/50 bg-gradient-to-r from-slate-50/50 to-transparent">
            ...
            <div className="p-4">
              <div className="space-y-3">
                {recentBookings.map((booking) => (
                  <div key={booking.id} className="group p-4 bg-gradient-to-r from-slate-50/50 to-transparent rounded-xl border border-slate-200/50 hover:shadow-lg transition-all duration-300 hover:scale-[1.02] cursor-pointer">
                    <div className="flex items-center space-x-3">
                      <div className="w-12 h-9 bg-gradient-to-br from-slate-200 to-slate-300 rounded-lg flex items-center justify-center overflow-hidden">
                        <img
                          src={booking.image}
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-bold text-slate-900 mt-1 text-base">{booking.amount}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-200/50 bg-gradient-to-r from-blue-50/50 to-transparent">
              <h3 className="text-lg font-bold text-slate-900">Upcoming Trips</h3>
              <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse"></div>
            </div>
            <div className="p-4 space-y-4">
              {upcomingTrips.map((trip) => (
                <div key={trip.id} className="group p-4 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl border border-blue-200/50 hover:shadow-lg transition-all duration-300 hover:scale-105 cursor-pointer">
                  <div className="w-full h-16 bg-gradient-to-br from-blue-200 to-cyan-300 rounded-lg mb-3 flex items-center justify-center overflow-hidden">
                    <img
                      src={trip.image}
                      alt={trip.property}
                      className="w-full h-full object-cover rounded-lg"
                  ...
                  <h4 className="font-bold text-slate-900 group-hover:text-blue-700 transition-colors mb-2 text-sm">{trip.property}</h4>
              ...
              {upcomingTrips.length === 0 && (
                <div className="text-center py-6">
                  ...
                  <button className="mt-2 px-3 py-1.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-all duration-200 text-xs font-medium hover:scale-105 shadow-lg">
                    Book Your Next Stay
                  </button>
                </div>
              )}
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-slate-900">Your Activity</h3>
            <TrendingUp className="w-5 h-5 text-emerald-500" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center p-4 bg-gradient-to-br from-slate-50 to-slate-100/50 rounded-xl border border-slate-200/50">
              <div className="w-10 h-10 bg-gradient-to-br from-emerald-500 to-green-600 rounded-xl flex items-center justify-center mx-auto mb-3 shadow-lg">
            ...
            <div className="text-center p-4 bg-gradient-to-br from-slate-50 to-slate-100/50 rounded-xl border border-slate-200/50">
              <div className="w-10 h-10 bg-gradient-to-br from-amber-500 to-orange-600 rounded-xl flex items-center justify-center mx-auto mb-3 shadow-lg">
                <Star className="w-5 h-5 text-white" />
              </div>
